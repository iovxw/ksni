...
impl<T: Tray> State<T> {
    ...
    fn update_properties(&self) {
        ...
        let text_direction = inner.text_direction();
        dbusmenu_changed.insert(
            "TextDirection".into(),
            Variant(Box::new(text_direction.to_string())),
        );

        let tray_status = inner.status();
        sni_changed.insert("Status".into(), Variant(Box::new(tray_status.to_string())));
        ...
        let icon_theme_path = inner.icon_theme_path();
        dbusmenu_changed.insert("IconThemePath".into(), Variant(Box::new(icon_theme_path.to_string())));
        sni_changed.insert("IconThemePath".into(), Variant(Box::new(vec![icon_theme_path.to_string()])));
        ...
        .to_emit_message(&service::SNI_PATH.into());
        self.tx.send(msg).unwrap();
    }
}
